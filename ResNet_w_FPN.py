...
def identity_block(input, channel_in, channel_out, kernel_size=3, stride=1):
    a = slim.conv2d(inputs=input, num_outputs=channel_in, kernel_size=1, stride=stride)
    a = slim.batch_norm(inputs=a, is_training=False, scale=True, epsilon=0)
    a = tf.nn.relu(a)

    b = slim.conv2d(inputs=a, num_outputs=channel_in, kernel_size=kernel_size, stride=1)
    b = slim.batch_norm(inputs=b, is_training=False, scale=True, epsilon=0)
    b = tf.nn.relu(b)

    c = slim.conv2d(inputs=b, num_outputs=channel_out, kernel_size=1, stride=1)
...
def branch_block(input, channel_in, channel_out, kernel_size=3, stride=2):
    a1 = slim.conv2d(inputs=input, num_outputs=channel_in, kernel_size=1, stride=stride)
    a1 = slim.batch_norm(inputs=a1, is_training=False, scale=True, epsilon=0)
    a1 = tf.nn.relu(a1)

    b1 = slim.conv2d(inputs=a1, num_outputs=channel_in, kernel_size=kernel_size, stride=1)
    b1 = slim.batch_norm(inputs=b1, is_training=False, scale=True, epsilon=0)
    b1 = tf.nn.relu(b1)

    c1 = slim.conv2d(inputs=b1, num_outputs=channel_out, kernel_size=1, stride=1)
    c1 = slim.batch_norm(inputs=c1, is_training=False, scale=True, epsilon=0)

    a2 = slim.conv2d(inputs=input, num_outputs=channel_out, kernel_size=1, stride=stride)
...
def forward(input):
    with tf.variable_scope('resnet_backbone'):
        C1 = slim.conv2d(inputs=input, num_outputs=64, kernel_size=7, padding='SAME', stride=2)
        ...
        # FPN
        P5 = slim.conv2d(inputs=C5, num_outputs=256, kernel_size=1)
        P4 = keras.layers.UpSampling2D()(P5) + slim.conv2d(inputs=C4, num_outputs=256, kernel_size=1)
        P3 = keras.layers.UpSampling2D()(P4) + slim.conv2d(inputs=C3, num_outputs=256, kernel_size=1)
        P2 = keras.layers.UpSampling2D()(P3) + slim.conv2d(inputs=C2, num_outputs=256, kernel_size=1)

        # reduce upsampling effects
        P2 = slim.conv2d(inputs=P2, num_outputs=256, kernel_size=3)
        P3 = slim.conv2d(inputs=P3, num_outputs=256, kernel_size=3)
        P4 = slim.conv2d(inputs=P4, num_outputs=256, kernel_size=3)
        # unnecessary for P5, but for consistency
        P5 = slim.conv2d(inputs=P5, num_outputs=256, kernel_size=3)
